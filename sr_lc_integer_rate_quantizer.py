...
class SR_LC_Int_Quantizer:
    ...
    def __init__(self, memory, rate, lc_coeff, mu, s, c_scale=1, source="normal", modulo_a=0, beta=1, distortion_measure='mse'):
        self.rate = rate
        self.memory = memory
        self.mu = mu
        self.s = s
        ...
        elif self.source == "dithered uniform" or self.source == "uniform":
            self.y = [self.c_sf * (self.s*np.sqrt(3)*(2*p-1) + self.mu) for p in q]
        elif self.source == "gennorm":
            self.y = self.c_sf * stats.gennorm.ppf(q, beta, loc=mu, scale=s)
    ...
    def distA(self, x, xh):
        ...
        if dist > 0:
            b = dist // self.a
            if np.abs(dist - self.a*b) > np.abs(dist - self.a*(b+1)):
                b = b + 1
        elif dist < 0:
            b = dist // self.a + 1
            if np.abs(dist - self.a*b) > np.abs(dist - self.a*(b-1)):
        ...
        for i in range(n):
            for state in self.states:
                for k in range(len(state.next)):
                    ...
                    #                    branch metric
                    if self.source == "dithered uniform":
                        bm = self.distA(x[i], rxn)
                    else:
